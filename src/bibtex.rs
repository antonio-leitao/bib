// NEW: Import necessary items for hashing and deterministic ordering
...
/// MODIFIED: Represents parsed BibTeX entry data with a content-based ID
...
/// assert_eq!(result.author, "Doe and Smith");
...
pub fn process_bibtex_entry(bibtex_str: &str) -> Result<BibtexData, BibtexError> {
    ...
    // MODIFIED: 'id' is now 'key' for clarity
    ...
    Ok(BibtexData {
        key,
        content_id, // Add the new ID here
        author,
        year,
        title,
        url,
...
/// Parses the URL field from a BibTeX entry
fn parse_url(entry: &Entry) -> Option<String> {
    // FIXED: Use the url() method and handle the Result<String, RetrievalError>
    entry.url().ok()
}
...
mod tests {
    ...
    fn test_id_stability() {
        ...
        // Same content, but with different field order and spacing
        let bib2 = r#"
        @article{knuth1984,
            title = "Literate Programming",
            year = "1984",
        ...
        let result1 = process_bibtex_entry(bib1).unwrap();
        let result2 = process_bibtex_entry(bib2).unwrap();

        // The keys are the same
        assert_eq!(result1.key, result2.key);
        // And more importantly, the content-based IDs are identical!
        assert_eq!(result1.content_id, result2.content_id);

        println!("ID for bib1: {}", result1.content_id);
        println!("ID for bib2: {}", result2.content_id);
    }
}
